function synchronization_sensitivity(;methods=["spectral", "sphere", "dyadic", "least-squares-orthogonal", "gaia", "crossproduct", "euclidean", "weiszfeld" ],vary_parameter="noise", σ_fixed=0.0, num_trials=Int(1e3), parameter_min=0.0, parameter_range=0.1, parameter_max=1.0, error=angular_distance)
    ...
    if occursin("noise", lowercase(vary_parameter))
        ...
        for σ=tqdm(parameter_min:parameter_range:parameter_max)
            ...
            for j=tqdm(1:num_trials)
                Eⱼ = create_synthetic(σ, averaging_methods=methods, error=error)    
    ...
    elseif occursin("density", lowercase(vary_parameter)) && !occursin("outliers", lowercase(vary_parameter))
        ...
        for ρ=tqdm(parameter_min:parameter_range:parameter_max)
            Eᵢ = Vector{Vector{Float64}}(undef, num_trials)
            for j=tqdm(1:num_trials)
                Eⱼ = create_synthetic(σ_fixed, averaging_methods=methods, error=error, holes_density=ρ)    
                if error==angular_distance
                    e = rad2deg.(mean.(eachcol(Eⱼ)))
                else
                    e = mean.(eachcol(Eⱼ))
    ...
    elseif occursin("frames", lowercase(vary_parameter))
        ...
        for n=tqdm(parameter_min:parameter_range:parameter_max)
            Eᵢ = Vector{Vector{Float64}}(undef, num_trials)
            for j=tqdm(1:num_trials)
                Eⱼ = create_synthetic(σ_fixed, averaging_methods=methods, error=error, frames=n)    
                if error==angular_distance
                    e = rad2deg.(mean.(eachcol(Eⱼ)))
                else
                    e = mean.(eachcol(Eⱼ))
    ...
    elseif occursin("outliers", lowercase(vary_parameter))
        ...
        for Ρ=tqdm(parameter_min:parameter_range:parameter_max)
            Eᵢ = Vector{Vector{Float64}}(undef, num_trials)
            for j=1:num_trials
                Eⱼ = create_synthetic(σ_fixed, averaging_methods=methods, error=error, outliers=Ρ)    
                if error==angular_distance
                    e = rad2deg.(mean.(eachcol(Eⱼ)))
                else
                    e = mean.(eachcol(Eⱼ))
...
# all_methods = ["spectral"; avg_methods]
# E_noise = synchronization_sensitivity(methods=avg_methods, parameter_min=0.0, parameter_range=0.05, parameter_max=0.2, num_trials=100, error=orthogonal_projection_distance);
