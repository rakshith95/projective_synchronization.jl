function weiszfeld(M::AbstractMatrix; normalize=true, max_iterations=1e3, initialize=nothing, c₀=nothing, δ=1e-6, σ=1e-3, Δ=rand(Distributions.Normal(0, σ), size(M)[1]) )
