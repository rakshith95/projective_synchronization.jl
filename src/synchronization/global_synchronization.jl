...
function projectivity_synch_spectral(Z::AbstractMatrix{Projectivity}, weights::AbstractArray{T}) where T<:AbstractFloat
    # Based on spectral solution for synchronization in GL(d) : https://iris.unitn.it/retrieve/handle/11572/272068/360005/ijcv20.pdf
...
function spanning_tree_synchronization(Z::AbstractMatrix{Projectivity})
    ...
    for e in ST
        ...
        for p in reverse(path)
            X[p] = Projectivity( inv(Z[ ST[p-1].src , p])*X[ST[p-1].src] )
        end
        
        X[dst] = Projectivity( inv(Z[e.src , dst])*X[e.src] )
    end 

    return X
end

function spanning_tree_synchronization(Z::AbstractMatrix{Projectivity}, weights::AbstractArray{T}) where T<:AbstractFloat
    n = size(Z,1)
    dims = size(Z[findfirst(!zero, Z)].P, 1)    
    X = SizedVector{n, Projectivity}(repeat([Projectivity(SMatrix{dims,dims,Float64}(I) )], n)) 
